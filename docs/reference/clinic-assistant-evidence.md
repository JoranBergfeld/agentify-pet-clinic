...
./mvnw -q -Dtest='ClinicQueryServiceTests,ClinicAssistantToolsTests,ClinicAssistantModelTests,ClinicAssistantConversationTests,ClinicAssistantBoundaryTests,ClinicAssistantServiceTests,ClinicAssistantControllerTests,ClinicAssistantSessionListenerTests,I18nPropertiesSyncTest' test
./mvnw -q test
```
...
- Focused assistant suite: PASS — 36 tests, 0 failures, 0 errors, 0 skipped
- Full Maven suite (`./mvnw -q test`): PASS — 25 suite reports, 105 tests, 0 failures, 0 errors, 2 skipped
...
The focused local suite directly asserts the configured `ClinicAssistantConfiguration.SYSTEM_PROMPT` boundary for unsupported or absent requests, read-only/no-mutation claims, veterinary diagnosis or treatment refusal, and multi-match clarification. This is PASS as configuration boundary evidence only; it does **not** claim live-model compliance.
