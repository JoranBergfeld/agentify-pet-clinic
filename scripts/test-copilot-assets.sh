...
evidence_coach_contract_mutations=(
  ...
  '- `Exit question`|- `Exit`'
  "Reject unrelated Markdown files or cards missing any required heading and produce no review.|Review unrelated Markdown files."
  ...
  "If revision resolution or prefix validation fails, any path is invalid, any required object is not a blob, any Stage Card guidance heading is absent, or committed content is unavailable, request corrected input and produce no review.|If committed evidence is missing, provide a best-effort review."
)
...
evidence_coach_scenario_mutations=(
  ...
  "**Expected behavior:** Verify that the object is a blob, then reject it because it lacks one or more required Stage Card headings Purpose, Risk controlled, Minimum evidence, Optional Copilot example, and Exit question; request corrected input and produce no review.|**Expected behavior:** Review the Markdown file."
