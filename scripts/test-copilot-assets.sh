...
evidence_coach_contract_mutations=(
  ...
  'Accept a commit SHA only when it matches `^[0-9a-fA-F]{7,40}$`; after this strict validation, verify that it names a commit with the read-only command `git rev-parse --verify "${sha}^{commit}"`.|Accept any Git revision.'
  'Each Stage Card path must be repository-relative, must not start with `-` or `/`, and must contain no `..` path segment.|Accept any Stage Card path.'
  'Use only the read-only Git commands needed to verify the commit and read committed content. Quote the single revision-and-path object argument when reading each card: `git show --no-ext-diff --format= "${sha}:${path}"`.|Use Git or shell commands as needed.'
  'Read the Evidence Lenses blueprint from the same named commit with `git show --no-ext-diff --format= "${sha}:docs/workshop-blueprint.md"`.|Read the working-tree blueprint.'
  ...
  "If the revision, any path, the committed blueprint, or any committed Stage Card is unavailable or invalid, request corrected input and produce no review.|If committed evidence is missing, provide a best-effort review."
)
