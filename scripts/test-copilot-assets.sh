...
evidence_coach_contract_mutations=(
  ...
  '- `What to record`|- `Evidence`'
  '- `Optional Copilot help`|- `Copilot example`'
  '- `Check before the next step`|- `Exit`'
  'Each required guidance section must appear as an actual Markdown heading line outside fenced code blocks, exactly `## Goal`, `## What can go wrong`, `## What to record`, `## Optional Copilot help`, or `## Check before the next step`.|Accept heading names anywhere in the blob.'
...
evidence_coach_scenario_mutations=(
  ...
  "**Request:** Review a committed Markdown blob where Goal, What can go wrong, What to record, Optional Copilot help, and Check before the next step occur only as headings inside a fenced code block.|**Request:** Review any Markdown blob containing the heading names."
...
stakeholder_grounding_mutations=(
  ...
  "Answer in a natural stakeholder voice first, then clearly distinguish any relevant **Fixed facts**, **Available preferences**, and **Explicit unknowns**. Do not force empty categories or repeat the knowledge document mechanically.|Summarize the available information in each answer."
...
stakeholder_voice_mutations=(
  "Speak as the clinic stakeholder: use concise, first-person language about what staff need, prefer, or do not know.|Report stakeholder information."
  "Use natural transitions such as \"What I know,\" \"My preference,\" and \"I don't know that yet\" instead of report-style headings. Translate engineering terms into clinic language unless a named boundary must be explicit.|Use report-style headings and engineering terminology."
  "Explain operational needs and consequences. Do not sound like an engineering agent, recite governance language, or prescribe implementation details.|Explain technical requirements and prescribe implementation details."
  "When a practical follow-up would clarify staff workflow or user impact, ask one focused question.|Do not ask follow-up questions."
)
for mutation in "${stakeholder_voice_mutations[@]}"; do
  IFS='|' read -r contract replacement <<<"$mutation"
  expect_agent_contract_mutation "$contract" "$replacement"
done
...
stakeholder_voice_behavior="**Expected behavior:** Lead with a concise first-person stakeholder answer grounded in clinic staff needs. Explain operational consequences rather than prescribing implementation, and identify relevant fixed facts, preferences, or unknowns without mechanically listing empty categories."
expect_scenario_mutation "## Stakeholder voice" "## Assistant voice"
expect_scenario_mutation \
  "$stakeholder_voice_behavior" \
  "**Expected behavior:** Recite all requirements and tell Engineering how to implement the capability."
