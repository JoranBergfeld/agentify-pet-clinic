...
local_setup="$root/docs/workshop/local-setup.md"
participant_guide="$root/docs/workshop/participant-guide.md"
...
test -f "$guide"
test -f "$local_setup"
test -f "$participant_guide"
test -f "$research"
test -f "$workflow"
test -f "$devcontainer"

grep -Fq 'docs/workshop/local-setup.md' "$readme"
grep -Fq 'docs/workshop/azure-preflight-and-cleanup.md' "$readme"
grep -Fq '## Option 1: Run on your computer' "$local_setup"
grep -Fq '## Option 2: Run in a Codespace' "$local_setup"
grep -Fq '## Prove partner review access' "$local_setup"
grep -Fq 'Azure Developer CLI' "$local_setup"
grep -Fq '(`azd`)' "$local_setup"
! grep -Fq '## Accounts and local tools' "$guide"
! grep -Fq '## Prove partner review access' "$guide"
! grep -Fq './mvnw' "$guide"
! grep -Fq './mvnw' "$readme"
! grep -Fq 'az login' "$local_setup"
! grep -Fq 'azd auth login' "$local_setup"
grep -Fq 'local-setup.md#prove-partner-review-access' "$participant_guide"
! grep -Fq 'gh api --method PUT' "$participant_guide"
grep -Fq 'ghcr.io/azure/azure-dev/azd:0' "$devcontainer"
...
grep -Fq 'Preflight keeps them running for the workshop' "$guide"
